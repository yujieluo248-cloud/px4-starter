...
class OffboardWideGateTraversalV4(Node):
    TIMER_PERIOD_S = 0.05
    PRESTREAM_TICKS = 20
    ...
    MIN_TAKEOFF_REL_HEIGHT_M = 0.48
    TAKEOFF_Z_TOL_M = 0.08
    TAKEOFF_XY_TOL_M = 0.20
    TAKEOFF_STABLE_SECONDS = 1.00
    TAKEOFF_TIMEOUT_SECONDS = 25.00

    GATE_YAW_RAD = 0.0
    MAX_YAW_ERROR_RAD = math.radians(8.0)
    YAW_STABLE_SECONDS = 1.0
    YAW_ALIGN_TIMEOUT_SECONDS = 10.0

    MAX_PRE_GATE_LATERAL_ERROR_M = 0.10
    MAX_PRE_GATE_VERTICAL_ERROR_M = 0.12
    ALIGNMENT_STABLE_SECONDS = 0.80
    ALIGNMENT_CHECK_TIMEOUT_S = 6.00
    ...
    def __init__(self) -> None:
        super().__init__("offboard_wide_gate_traversal_v4")
        ...
        self.vehicle_attitude: Optional[VehicleAttitude] = None
        ...
        self.yaw_stable_since: Optional[float] = None
        self.alignment_stable_since: Optional[float] = None
        self.last_log = 0.0
        ...
        self.get_logger().info(
            "Wide-gate traversal v4 started. "
    ...
    def _on_vehicle_attitude(self, msg: VehicleAttitude) -> None:
    ...
    def _publish_setpoint(self) -> None:
        ...
        if self.phase in (
            ...
            msg.yaw = float(self.GATE_YAW_RAD)
        else:
            msg.yaw = nan
        msg.yawspeed = nan
    ...
    def _set_phase(self, phase: Phase, message: str) -> None:
        ...
        if phase == Phase.YAW_ALIGN and self._has_valid_position():
            self.target = self._position()
            self.target_velocity = None
            self.yaw_stable_since = None
    ...
    def _phase_timeout_seconds(self) -> float:
        ...
        if self.phase == Phase.YAW_ALIGN:
            return self.YAW_ALIGN_TIMEOUT_SECONDS
        if self.phase in (
            Phase.SMOOTH_APPROACH,
            Phase.SMOOTH_GATE_CROSS,
        ):
    ...
    def _current_yaw_with_source(self) -> Tuple[Optional[float], str]:
        if self.vehicle_attitude is not None and len(self.vehicle_attitude.q) == 4:
            q = [float(value) for value in self.vehicle_attitude.q]
            if all(math.isfinite(value) for value in q):
                ...
                if norm > 1.0e-12:
                    ...
                    return yaw, "vehicle_attitude"

        if (
            self.local_position is not None
            and math.isfinite(self.local_position.heading)
        ):
            return float(self.local_position.heading), "local_position_heading"

        return None, "invalid"

    def _current_yaw(self) -> Optional[float]:
        yaw, _ = self._current_yaw_with_source()
        return yaw

    def _yaw_error(self) -> Optional[float]:
        current_yaw = self._current_yaw()
        if current_yaw is None:
            return None
        return math.atan2(
            math.sin(self.GATE_YAW_RAD - current_yaw),
            math.cos(self.GATE_YAW_RAD - current_yaw),
        )
    ...
    def _log_status(self) -> None:
        ...
        if self.phase == Phase.YAW_ALIGN:
            current_yaw, yaw_source = self._current_yaw_with_source()
            yaw_error = self._yaw_error()
            current_yaw_text = "invalid" if current_yaw is None else f"{current_yaw:.3f}"
            yaw_error_text = "invalid" if yaw_error is None else f"{yaw_error:.3f}"
            local_heading = float(self.local_position.heading)
            self.get_logger().info(
                f"YAW_ALIGN: yaw_source={yaw_source}, current_yaw={current_yaw_text}, "
                f"target_yaw={self.GATE_YAW_RAD:.3f}, yaw_error={yaw_error_text}, "
                f"local_position.heading={local_heading:.3f}, "
                f"heading_good_for_control="
                f"{bool(self.local_position.heading_good_for_control)}, "
    ...
    def _yaw_alignment_ready(self) -> bool:
        if (
            not self._has_valid_position()
            or not self._has_offboard_and_arm()
            or self.vehicle_status is None
            or bool(self.vehicle_status.failsafe)
        ):
            self.yaw_stable_since = None
            return False

        yaw_error = self._yaw_error()
        if yaw_error is None or abs(yaw_error) > self.MAX_YAW_ERROR_RAD:
            self.yaw_stable_since = None
            return False

        now = time.monotonic()
        if self.yaw_stable_since is None:
            self.yaw_stable_since = now
        if now - self.yaw_stable_since < self.YAW_STABLE_SECONDS:
            return False

        self.get_logger().info(
            "YAW_ALIGNMENT_PASSED: "
            f"current_yaw={self._current_yaw():.3f}, "
            f"target_yaw={self.GATE_YAW_RAD:.3f}, yaw_error={yaw_error:.3f}"
        )
        return True

    def _yaw_alignment_failed_to_land(self) -> None:
        current_yaw = self._current_yaw()
        yaw_error = self._yaw_error()
        current_yaw_text = "invalid" if current_yaw is None else f"{current_yaw:.3f}"
        yaw_error_text = "invalid" if yaw_error is None else f"{yaw_error:.3f}"
        self.get_logger().error(
            "YAW_ALIGNMENT_FAILED: requesting AUTO LAND. "
            f"current_yaw={current_yaw_text}, target_yaw={self.GATE_YAW_RAD:.3f}, "
            f"yaw_error={yaw_error_text}, arming_state={self._arming_state()}, "
            f"nav_state={self._nav_state()}, failsafe={self._failsafe_state()}"
        )
        self.request_land()
        self._set_phase(Phase.LAND, "LAND: yaw alignment failed.")
    ...
    def _pre_gate_alignment_ready(self) -> bool:
        ...
        if now - self.alignment_stable_since >= self.ALIGNMENT_STABLE_SECONDS:
            self.get_logger().info(
    ...
    def _tick(self) -> None:
        ...
        if self.phase not in (Phase.LAND, Phase.DONE):
            ...
            if self._phase_timed_out():
                ...
                if self.phase == Phase.TAKEOFF:
                    ...
                    return
                if self.phase == Phase.YAW_ALIGN:
                    self._yaw_alignment_failed_to_land()
                    return
                self._timeout_to_land()
                return
        ...
        if self.phase == Phase.TAKEOFF:
            if self._takeoff_ready():
                self._set_phase(
                    Phase.YAW_ALIGN,
                    "YAW_ALIGN: holding position and aligning yaw toward local +X.",
                )
                return
            return

        if self.phase == Phase.YAW_ALIGN:
            if self._yaw_alignment_ready():
...
def main(args=None) -> None:
    ...
    node = OffboardWideGateTraversalV4()
    try:
        rclpy.spin(node)
