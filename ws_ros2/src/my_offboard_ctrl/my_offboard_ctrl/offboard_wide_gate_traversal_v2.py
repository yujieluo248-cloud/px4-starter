#!/usr/bin/env python3
"""
Wide-gate traversal v2.

This node performs one conservative, staged traversal through the wide gate.
It uses only PX4 local coordinates relative to a stable takeoff reference.
"""
...
class Phase(Enum):
    ...
    PRE_GATE_HOLD = auto()
    MOVE_TO_GATE_ENTRY = auto()
    GATE_ENTRY_HOLD = auto()
    CROSS_GATE = auto()
...
class OffboardWideGateTraversalV2(Node):
    TIMER_PERIOD_S = 0.10
    PRESTREAM_TICKS = 20
    ...
    GATE_ENTRY_DX_M = 1.85
    GATE_ENTRY_DY_M = -0.45

    AFTER_GATE_DX_M = 2.75
    AFTER_GATE_DY_M = -0.45

    PRE_GATE_HOLD_SECONDS = 1.50
    GATE_ENTRY_HOLD_SECONDS = 0.80
    AFTER_GATE_HOLD_SECONDS = 2.00
    ...
    def __init__(self) -> None:
        super().__init__("offboard_wide_gate_traversal_v2")
        ...
        self.cross_gate_start_offset: Optional[Tuple[float, float, float]] = None
        ...
        self.get_logger().info(
            "Wide-gate traversal v2 started. "
            "This run uses pre-gate, gate-entry, direct cross, and after-gate holds."
        )
        self.get_logger().info(
            f"TAKEOFF_UP_M={self.TAKEOFF_UP_M:.2f}, "
            f"PRE_GATE=({self.PRE_GATE_DX_M:.2f}, {self.PRE_GATE_DY_M:.2f}), "
            f"GATE_ENTRY=({self.GATE_ENTRY_DX_M:.2f}, {self.GATE_ENTRY_DY_M:.2f}), "
    ...
    def _set_phase(self, phase: Phase, message: str) -> None:
        ...
        if phase in (Phase.MOVE_TO_PRE_GATE, Phase.MOVE_TO_GATE_ENTRY):
            self.move_started = time.monotonic()
            self.move_start_offset = self._current_offset()
        if phase == Phase.CROSS_GATE:
    ...
    def _hold_complete_after_reach(self, seconds: float) -> bool:
        if not self._stable_at_target():
            return False
        if self.hold_started is None:
            self.hold_started = time.monotonic()
        return time.monotonic() - self.hold_started >= seconds
    ...
    def _flight_mode_lost_to_land(self) -> bool:
        if self.phase not in (
            ...
            Phase.PRE_GATE_HOLD,
            Phase.MOVE_TO_GATE_ENTRY,
            Phase.GATE_ENTRY_HOLD,
            Phase.CROSS_GATE,
    ...
    def _horizontal_progress(self) -> Tuple[float, float]:
        if (
            self.move_start_offset is None
    ...
    def _horizontal_response_watchdog_to_land(self) -> bool:
        if (
            self.phase not in (Phase.MOVE_TO_PRE_GATE, Phase.MOVE_TO_GATE_ENTRY)
    ...
    def _tick(self) -> None:
        ...
        if self.phase == Phase.TAKEOFF:
            if self._takeoff_ready():
                ...
                self._set_phase(
                    Phase.MOVE_TO_PRE_GATE,
                    "MOVE_TO_PRE_GATE: moving to pre-gate alignment point.",
                )
            return
        ...
        if self.phase == Phase.MOVE_TO_PRE_GATE:
            ...
            if self._stable_at_target():
                self._set_phase(
                    Phase.PRE_GATE_HOLD,
                    "PRE_GATE_HOLD: holding pre-gate point.",
                )
            return

        if self.phase == Phase.PRE_GATE_HOLD:
            if self._hold_complete_after_reach(self.PRE_GATE_HOLD_SECONDS):
                self.target = self._target_from_home(
                    self.GATE_ENTRY_DX_M,
                    self.GATE_ENTRY_DY_M,
                )
                self._set_phase(
                    Phase.MOVE_TO_GATE_ENTRY,
                    "MOVE_TO_GATE_ENTRY: moving to gate-entry point.",
                )
            return

        if self.phase == Phase.MOVE_TO_GATE_ENTRY:
            if self._horizontal_response_watchdog_to_land():
                return
            if self._stable_at_target():
                self._set_phase(
                    Phase.GATE_ENTRY_HOLD,
                    "GATE_ENTRY_HOLD: holding briefly before direct crossing.",
                )
            return

        if self.phase == Phase.GATE_ENTRY_HOLD:
            if self._hold_complete_after_reach(self.GATE_ENTRY_HOLD_SECONDS):
                self.target = self._target_from_home(
                ...
                self._set_phase(
                    Phase.CROSS_GATE,
                    "CROSS_GATE: crossing directly to after-gate point.",
                )
            return

        if self.phase == Phase.CROSS_GATE:
            ...
            if self._stable_at_target():
                ...
                self._set_phase(
                    Phase.AFTER_GATE_HOLD,
                    "AFTER_GATE_HOLD: holding after gate.",
                )
            return

        if self.phase == Phase.AFTER_GATE_HOLD:
            if self._hold_complete_after_reach(self.AFTER_GATE_HOLD_SECONDS):
                self.request_land()
...
def main(args=None) -> None:
    ...
    node = OffboardWideGateTraversalV2()
    try:
        rclpy.spin(node)
